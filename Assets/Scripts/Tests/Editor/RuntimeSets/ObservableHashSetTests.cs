...
namespace Tests.RuntimeSet {
  public class ObservableHashSetTests {
    private ObservableHashSet<int> set;
    ...
    [SetUp]
    public void SetUp() {
      set = ScriptableObject.CreateInstance<ObservableHashSet<int>>();
    ...
    public void GetItemAt_ReturnsCorrectItem() {
      set.Add(1);
      set.Add(2);
      // Since HashSet doesn't guarantee order, we just check that we get valid items
      object item1 = set.GetItemAt(0);
      object item2 = set.GetItemAt(1);
      Assert.IsNotNull(item1);
      Assert.IsNotNull(item2);
      Assert.IsTrue(item1.Equals(1) || item1.Equals(2));
      Assert.IsTrue(item2.Equals(1) || item2.Equals(2));
